...
SELECT set_config('homex.stock_update_context','MOVIMIENTO_STOCK',true); UPDATE productos SET stock=999 WHERE id=90001; DO $$ BEGIN ASSERT (SELECT stock=999 FROM productos WHERE id=90001); RAISE NOTICE 'VERIFICADO'; END $$;
SELECT 'AUDIT_OK';
ROLLBACK;
