...
DO $$ BEGIN BEGIN UPDATE movimientos_stock SET cantidad=11 WHERE producto_id=90001; RAISE EXCEPTION 'NO_RECHAZADO'; EXCEPTION WHEN OTHERS THEN IF SQLERRM='NO_RECHAZADO' THEN RAISE; END IF; END; BEGIN DELETE FROM movimientos_stock WHERE producto_id=90001; RAISE EXCEPTION 'NO_RECHAZADO'; EXCEPTION WHEN OTHERS THEN IF SQLERRM='NO_RECHAZADO' THEN RAISE; END IF; END; RAISE NOTICE 'VERIFICADO'; END $$;
SELECT 'AUDIT_OK';
ROLLBACK;
