...
UPDATE proformas_detalle SET cantidad=0.5 WHERE id=90001;UPDATE proformas SET estado_id=pg_temp.cv('ESTADO_PROFORMA','APROBADA') WHERE id=90001;DO $$ BEGIN ASSERT (SELECT stock=9.5 FROM productos WHERE id=90001); RAISE NOTICE 'VERIFICADO'; END $$;
SELECT 'AUDIT_OK';
ROLLBACK;
