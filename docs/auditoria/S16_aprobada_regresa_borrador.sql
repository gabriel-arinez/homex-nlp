...
UPDATE proformas SET estado_id=pg_temp.cv('ESTADO_PROFORMA','APROBADA') WHERE id=90001;UPDATE proformas SET estado_id=pg_temp.cv('ESTADO_PROFORMA','BORRADOR') WHERE id=90001; DO $$ BEGIN ASSERT EXISTS(SELECT 1 FROM pedidos WHERE proforma_id=90001); ASSERT (SELECT stock=8 FROM productos WHERE id=90001); RAISE NOTICE 'VERIFICADO'; END $$;
SELECT 'AUDIT_OK';
ROLLBACK;
