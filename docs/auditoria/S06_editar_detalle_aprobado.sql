...
UPDATE proformas SET estado_id=pg_temp.cv('ESTADO_PROFORMA','APROBADA') WHERE id=90001;UPDATE proformas_detalle SET cantidad=7 WHERE id=90001; DO $$ BEGIN ASSERT (SELECT stock=8 FROM productos WHERE id=90001); ASSERT (SELECT cantidad=7 FROM proformas_detalle WHERE id=90001); RAISE NOTICE 'VERIFICADO'; END $$;
SELECT 'AUDIT_OK';
ROLLBACK;
