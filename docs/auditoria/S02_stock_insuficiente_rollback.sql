...
UPDATE proformas_detalle SET cantidad=11 WHERE id=90001; DO $$ BEGIN BEGIN UPDATE proformas SET estado_id=pg_temp.cv('ESTADO_PROFORMA','APROBADA') WHERE id=90001; RAISE EXCEPTION 'NO_RECHAZADO'; EXCEPTION WHEN OTHERS THEN IF SQLERRM='NO_RECHAZADO' THEN RAISE; END IF; END; ASSERT (SELECT stock=10 FROM productos WHERE id=90001); ASSERT NOT EXISTS(SELECT 1 FROM pedidos); RAISE NOTICE 'VERIFICADO'; END $$;
SELECT 'AUDIT_OK';
ROLLBACK;
